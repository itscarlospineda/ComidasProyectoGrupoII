...
<title>@yield('title', 'Lista de Posts - Sabor Catracho')</title>
        ...
        <tbody>
            @foreach ($posts as $post)
                <tr>
                    ...
                    <td>
                        @if($post->picture1)
                            <img src="{{ asset('storage/' . $post->picture1) }}" alt="{{ $post->title }}" class="post-image">
                        @else
                    ...
                    <td>
                                ...
                                @csrf
                                <a href="{{ route('posts.edit', $post->id) }}" class="btn-action editar" style="background-color: #007bff; color: white; text-decoration: none; padding: 8px 15px; border-radius: 5px;">Editar</a>
                            </form>
