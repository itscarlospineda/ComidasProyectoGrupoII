...
<title>@yield('title', 'Editar Recompensa - Sistema de Recompensas')</title>
    ...
    <div class="form-section">
        ...
        <form action="{{ route('rewards.update', $reward->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <textarea id="Description" name="Description" required>{{ old('Description', $reward->Description) }}</textarea>
                @error('Description')
                    <small style="color: #e74c3c;">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="Points_needed" name="Points_needed" value="{{ old('Points_needed', $reward->Points_needed) }}" required>
                @error('Points_needed')
                    <small style="color: #e74c3c;">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="form-actions">
                <button type="submit">Actualizar Recompensa</button>
            </div>
        </form>
    </div>
@endsection
